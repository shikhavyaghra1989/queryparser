...
public class QueryParserController {
    ...
    public ResponseEntity<String> executeQuery(@RequestBody QueryRequest request) {
        ...
        try{
            ...
            if(list[0].equalsIgnoreCase(QueryType.SELECT.getKey()) || list[0].equalsIgnoreCase(QueryType.SELECT.getValue())
                || list[0].equalsIgnoreCase(QueryType.DESC.getKey()) || list[0].equalsIgnoreCase(QueryType.DESC.getValue())) {
                System.out.println("In first if");
                Response result = service.executeSelectQuery(request);
                return new ResponseEntity<>(result.getResponse(),result.getStatus());
            } else if(list[0].equalsIgnoreCase(QueryType.UPDATE.getKey()) || list[0].equalsIgnoreCase(QueryType.UPDATE.getValue())
                        || list[0].equalsIgnoreCase(QueryType.DELETE.getKey()) || list[0].equalsIgnoreCase(QueryType.DELETE.getValue())
                        || list[0].equalsIgnoreCase(QueryType.INSERT.getKey()) || list[0].equalsIgnoreCase(QueryType.INSERT.getValue())){
                System.out.println("In second if");
                Response result = service.executeDataManipulationQuery(request);
                return new ResponseEntity<>(result.getResponse(),result.getStatus());
            } else  if(list[0].equalsIgnoreCase(QueryType.DROP.getKey()) || list[0].equalsIgnoreCase(QueryType.DROP.getValue())
                    || list[0].equalsIgnoreCase(QueryType.CREATE.getKey()) || list[0].equalsIgnoreCase(QueryType.CREATE.getValue())
                    || list[0].equalsIgnoreCase(QueryType.ALTER.getKey()) || list[0].equalsIgnoreCase(QueryType.ALTER.getValue())){
